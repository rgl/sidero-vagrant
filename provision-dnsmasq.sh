...
# TFTP
# NB TFTP is handled by sidero instead of dnsmasq.
...
dhcp-boot=tag:efi64-http,tag:eth1,http://$ip_address/tftp/ipxe.efi
dhcp-match=set:efiarm64-http,option:client-arch,19 # ARM64 UEFI HTTP (19)
dhcp-option-force=tag:efiarm64-http,60,HTTPClient
dhcp-boot=tag:efiarm64-http,tag:eth1,http://$ip_address/tftp/ipxe-arm64.efi
...
python3 /vagrant/machines.py save-dnsmasq-machines
